...
public class Board {
    ...
    /**
     * Returns the two or more players with an equal and highest score or
     * null if the game has a winner.<br>
     * 
     * @return the 'drawing' Balls or null
     */  
    /*@
    requires !hasWinner();
    ensures (\exists Ball b; isPlayableBall(b) && 
                            !(\exists Ball other; isPlayableBall(other) && 
                                    countInstancesOf(b) >= countInstancesOf(other)); \result.contains(b));
    */
    public ArrayList<Ball> getDrawers() {
    	ArrayList<Ball> drawers = null;
    	if (getWinner() == null) {
    		drawers = new ArrayList<Ball>();
	    	int best = 0;
	    	for (Ball ball : Ball.values()) {
	    		if (isPlayableBall(ball)) {
	    			int temp = countInstancesOf(ball);
    ...
    /**
     * Removes all the instances of this ball from the board
     * @param b the ball of interest
     */
    /*@
    requires b != null;
    ensures (\forall int x, y; isField(x, y); getField(x , y) != b); 
   */
    public void removeBallFromBoard(Ball b) {
		for (int x = 0; x < Board.X_MAX; x++) {
			for (int y = 0; y < Board.Y_MAX; y++) {
				if (getField(x, y).equals(b)) {
					setField(new Point(x, y), Ball.EMPTY, false);
				}
			}
		}
    }
